...
form.addEventListener('submit', e => {
  ...
  try {
    ...
    resultEl.innerHTML = `
        ...
        <p><strong>Tlaková ztráta:</strong> ${lossBar} bar</p>
        <p style="color: var(--text-muted); font-size: 0.9em;">
          ${loss_Pa.toFixed(2)} Pa | ${(loss_Pa / 6894.76).toFixed(2)} psi
        </p>
        <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--border-color);">
            <p><strong>Tlakový spád (teoreticky):</strong> ${(nominal_Pa / 1e5).toFixed(4)} bar (${nominal_Pa.toFixed(0)} Pa)</p>
            <p><strong>Reynoldsovo číslo Re:</strong> ${Re.toFixed(0)}</p>
            <p><strong>Součinitel tření f:</strong> ${f.toFixed(4)}</p>
        </div>
      `;
    ...
    resultEl.style.display = 'block';
    resultEl.innerHTML = `<p style="color: var(--error-color);">Chyba výpočtu: ${error.message}</p>`;
  }
});
